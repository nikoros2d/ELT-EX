...
void *thread_customer(void *argc){
  int *ptr = (int *) argc;
  while(1){
  pthread_mutex_lock(&m[ptr[0]]);
  ptr[1]=ptr[1]-a[ptr[0]];
  a[ptr[0]]=0;
  if(ptr[1]==0){printf("FINISHED THREAD NUM %d, DEMAND = %d\n",ptr[0],ptr[1]);break;}
  sleep(1);
  sleep_customer++;
...
pthread_create(&thread[i],NULL, thread_customer, (void *) &custom[i-1]);
}
