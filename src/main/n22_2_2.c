...
pid_t child_proc[4];
short ports[4] = { 9801, 9802, 9803, 9804 };
          ...
          while (1)
            {
              ...
              recvfrom (proc_fd, proc_received, 256, 0,
                        (struct sockaddr *)&clienttosend, &lengthforchild);
          ...
          for (int i = 0; i < 4; i++)
            {
              ...
              if (!semctl (semaphore[i], 0, GETVAL, 0))
              ...
              else
                {
                  ...
                  if (-1
                      == sendto (request_fd, port_to_send,
                                 strlen (port_to_send), 0,
                                 (struct sockaddr *)&client, length))
                    {
                      perror ("sendto");
                      close (request_fd);
