...
char *
shtos (struct sockaddr_in server, int size)
{
      ...
      if (child_proc == 0)
        {
          ...
          if (-1
              == sendto (request_fd, sended_info, strlen (sended_info), 0,
          ...
          printf ("Child proc wait for Message...\n");
          recvfrom (proc_fd, proc_received, 256, 0, (struct sockaddr *)&client,
                    sizeof (struct sockaddr_in));
          ...
          if (-1
              == sendto (proc_fd, sended_info, 256, 0,
                         (struct sockaddr *)&client,
                         sizeof (struct sockaddr_in)))
            {
              perror ("sendto");
