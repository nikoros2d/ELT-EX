  ...
  while (1)
    {
      ...
      if (FD_ISSET (srv_sock_udp, &read_fds))
        {
          printf ("UDP ready for reading.\n");

          int received_bytes
              = recvfrom (srv_sock_udp, buffer, BUFF_SIZE, 0,
                          (struct sockaddr *)&cli_addr, &cli_len);
          if (received_bytes == -1)
            {
              perror ("UDP receive");
          ...
          client_count++;
          printf ("UDP Received: %s\n  from  client %d", buffer, client_count);
