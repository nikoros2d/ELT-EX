...
int main(int argc, char *argv[]) {
  ...
  if(sendto(fd,msg,sizeof(msg),0,(struct sockaddr*)&serv,addrlen)== -1){perror("send");exit(EXIT_FAILURE);};
  if(recvfrom(fd,msg,sizeof(msg),0,(struct sockaddr*)&serv,&addrlen)== -1){perror("recv");exit(EXIT_FAILURE);};
