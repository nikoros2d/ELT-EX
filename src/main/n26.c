...
int main() {
  ...
  ipheader->version = 4;
  ipheader->tos = 0;
  ipheader->tot_len = htons(sizeof(struct iphdr) + sizeof(struct udphdr) + strlen(msg));
  ipheader->id = htons(1);
  ipheader->frag_off = 0;
  ipheader->ttl = 254;
  ...
  memcpy(buffer +sizeof(struct ethhdr) + sizeof(struct iphdr) + sizeof(struct udphdr), msg,
         strlen(msg));

  if (sendto(raw_socket, buffer, sizeof(struct iphdr) + sizeof(struct udphdr) + sizeof(struct ethhdr) + strlen(msg), 0,
